...
const prettyQuickResult = (0, _1.default)(process.cwd(), Object.assign(Object.assign({}, args), { onFoundSinceRevision: (scm, revision) => {
        console.log(`🔍  Finding changed files since ${chalk_1.default.bold(scm)} revision ${chalk_1.default.bold(revision)}.`);
    }, onFoundChangedFiles: changedFiles => {
        console.log(`🎯  Found ${chalk_1.default.bold(changedFiles.length)} changed ${changedFiles.length === 1 ? 'file' : 'files'}.`);
    }, onPartiallyStagedFile: file => {
        console.log(`✗ Found ${chalk_1.default.bold('partially')} staged file ${file}.`);
    }, onWriteFile: file => {
        console.log(`✍️  Fixing up ${chalk_1.default.bold(file)}.`);
    }, onCheckFile: (file, isFormatted) => {
        if (!isFormatted) {
            console.log(`⛔️  Check failed: ${chalk_1.default.bold(file)}`);
        }
    }, onExamineFile: file => {
        console.log(`🔍  Examining ${chalk_1.default.bold(file)}.`);
    } }));
if (prettyQuickResult.success) {
    ...
    if (prettyQuickResult.errors.includes('PARTIALLY_STAGED_FILE')) {
        console.log('✗ Partially staged files were fixed up.' +
            ` ${chalk_1.default.bold('Please update stage before committing')}.`);
    }
